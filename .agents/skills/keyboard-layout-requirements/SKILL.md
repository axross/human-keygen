...
description: Use this skill when adding, editing, testing, or reviewing keyboard layout data and layout-derived character sets. Covers QWERTY reference ownership, printable literal characters, shifted and AltGr ambiguity, metadata, source notes, deterministic ordering, category derivation, fixtures, and tests that prove generated passwords stay typeable.
---
...
Layout data should be boring, explicit, and reviewable. A future contributor should be able to see why a character is available.

**Guidelines:**

- MUST define one QWERTY reference layout.
- MUST give each layout a stable kebab-case `id`, display label, family or locale note, character set, and source note.
- MUST represent supported characters as literal strings or arrays that can be normalized into a set.
...
- MUST derive letters, uppercase letters, digits, and symbols from the final layout intersection.
...
- MUST add tests for every new layout proving its intersection with QWERTY.
