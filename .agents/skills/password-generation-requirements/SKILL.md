...
The character pool is the product's core promise: generated passwords use only characters common to QWERTY and the selected layout.

**Guidelines:**

- MUST compute the pool as the literal character intersection between QWERTY and the selected layout.
- MUST apply user-selected categories only after computing the layout intersection.
...
Memorability depends on complete word chunks. Length handling must choose words that exactly fit instead of cutting off a final word or padding with random leftover characters.

**Guidelines:**

- MUST compose the base phrase from complete word candidates whose lengths sum exactly to the requested password length.
...
- MUST calculate estimated entropy from the actual whole-word generation space once word-list generation is used.
- MUST NOT display `length * log2(poolSize)` as the primary estimate for word-list generated passwords.
